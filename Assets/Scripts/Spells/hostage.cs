...
public class hostage : MonoBehaviour
{
    ...
    private CController _cController;
    private int index = 0;
    ...
    private void Awake()
    {
        ...
        _castSystem = _hero.GetComponent<CastSystem>();
        _cController = _hero.GetComponent<CController>();
    ...
    private void Update()
    {
        ...
        if (_isEndOfRoad)
        {
            transform.position = Vector3.MoveTowards(transform.position, new Vector3(transform.position.x, -10f, transform.position.z), 15f * Time.deltaTime);
            if (transform.position.y < 21f)
            {
                Destroy(gameObject);
                _cController.RunDieEvents();
            }
        }
    }
    
    public void RunDanceAnimation(CController player)
    {
        transform.LookAt(player.transform);

        animator.SetTrigger("isWinn");
    }
}
